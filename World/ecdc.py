...
def cases_by_country() -> list:
    ...
    df = pd.DataFrame(cases.json().get("records", [])).rename(columns={"countriesAndTerritories": COUNTRY})
    return df
...
def weekly(
    ...
    """
    cases[YEAR_WEEK] = cases.apply(lambda row: f"{row['year']}-W{week_number(to_date(row))}", axis=1)
    df_final = cases.groupby([COUNTRY, YEAR_WEEK])[[CASES, DEATHS]].agg('sum').reset_index()
    
    if testing is not None:
        testing_data = testing.set_index([COUNTRY, YEAR_WEEK])
        cols_to_use = testing.columns.difference(df_final.columns)
        df_final = pd.merge(df_final, testing_data[cols_to_use], on=[COUNTRY, YEAR_WEEK], how="outer").set_index([COUNTRY, YEAR_WEEK])
    
    if hospital_rates is not None:
        hospital_data = hospital_rates.groupby([COUNTRY, YEAR_WEEK])[[HOSPITAL_RATE]].agg('sum') # some countries have daily data for hospital rate
        cols_to_use = hospital_data.columns.difference(df_final.columns)
        df_final = pd.merge(df_final, hospital_data[cols_to_use], on=[COUNTRY, YEAR_WEEK], how="outer")
